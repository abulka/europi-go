...
func GetCharacterData(char byte) []uint8 {
	if char < 32 || char > 127 {
...
func DrawCharacter(display ssd1306.Device, x, y int16, char byte, c color.RGBA) {
	charData := GetCharacterData(char)
	if charData == nil {
...
func WriteLine(display ssd1306.Device, x, y int16, text string, c color.RGBA) {
	currentX := x
	for i := 0; i < len(text); i++ {
		...
		DrawCharacter(display, currentX, y, text[i], c)
...
func HighlightLine(display ssd1306.Device, x, y int16, text string, margin int16, fg, bg color.RGBA) {
